...
一个主要的障碍需要克服的是比特币术语中所谓的“双重花费攻击”：如果网络中存在两笔都想清空一个账户的交易会发生什么情况？只能有一笔交易是有效的，通常是第一笔被接受的交易。问题是在P2P网络中，“第一笔”并不是一个客观的定义。

解决它的方式就是比特币本身,比特币是一种分布式账本,所有交易会被以块的方式分发给其他人,然后超过51%同意的会被加到块中-也就是这个分布式帐本中. 所以比特币实际上不需要在意这个问题

因为一个快是否应该被加入又共识算法决定?而共识算法是矿工出算力,矿工出算力,用的是共识算法,解决了公式问题,矿工得到了coin.

## 51%问题

矿工或一组矿工试图控制超过50％的网络挖掘能力，计算能力或哈希率。在此攻击中，攻击者可以阻止新事务发生或被确认。他们还可以撤销在他们控制网络时已经确认的交易，从而导致双重支出问题。
...
![img](https://www.yiibai.com/uploads/article/2019/06/21/104226_92866.png)

#### 1. 了解比特币

比特币有一些特殊的品质。它的行为很像金钱，但它的行为方式却有所不同。例如：使用比特币，交易无法逆转。区块链网络中的每笔交易都是永久性的。这使得它与实际向银行汇款的情况不同，您可以在银行发出停止付款或发出逆转付款的错误付款。与实物货币相比，比特币也快速有效。

#### 2. 选择钱包

在了解了比特币之后，需要做的第一件事就是安装钱包。这是您在移动设备或计算机上安装的一种软件。它使您能够进入比特币并创建自己的地址。
...
热钱包是一个软件.
推荐热钱包:
MetaMask for ETH(**Ethereum** )
Electrum for BTC(Bitcoin)

#### 3. 获取地址

比特币中的地址与电子邮件地址非常相似。该地址用于接收比特币。与只有一个电子邮件地址的电子邮件地址不同，您在比特币中可以使用多个地址。为不同的需求提供多个地址是一种很好的做法。也可以在每次单笔交易中使用单个比特币地址。

#### 4. 公钥和私钥

当打开钱包时，您将收到两条有价值的信息。其中一个是公钥，另一个是私钥。这使得比特币更有趣。对于大多数企业和不同的组织，您需要拥有一个密码，可以与实际运营该组织的人员共享密码。您可以单击“检索我的密码”，然后将其恢复。在比特币中并非如此，在这里你得到钥匙。可以与每个人共享的公钥，但是私钥只有您知道(私有)。私钥是一个字母数字序列，只有知道并且与保存好它们，私钥用来进入钱包和完全控制所有资金。

#### 5. 获得比特币

一旦拿到钱包，就可以获得比特币。获得比特币的方法有很多种。你可以通过做一份工作或获得报酬来赚取比特币。也可以购买比特币，这意味着您可以通过将美元或任何货币投入比特币来进入交易所并获得比特币。也可以赠送比特币，也可以从别人那里收到。这些是获取比特币的方法。



#### 6. 花费比特币

如何真正花费比特币？您可以将比特币发送给任何人。目前在全球范围内使用比特币超过160000家企业。许多知名品牌微软，戴尔都将比特币作为付款方式。还可以使用比特币发送付款。可向全球不同货币的人发送付款，这将非常有效。您不必担心必须更改为不同的货币。可以用比特币支付，它在全世界都是普遍的。
